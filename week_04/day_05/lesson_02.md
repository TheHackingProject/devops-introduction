...
de gestion de sauvegardes en cas de passe ou de crash, etc.  
...
- Lors de la création de ton instance RDS, tu dois choisir le type d'instance (un serveur où sera installer ta DB) que tu souhaites,
  et comme tu l'as compris le service qui se cache derrière ces instances, c'est le service EC2 🙂. 

  Mais contrairement à Elastic Beanstalk, tu ne peux pas accéder aux instances EC2 créer par RDS, 
  parce que c'est un service managé, et pour qu'AWS pour qu'ils puissent garantir toutes cees notions ci-dessus, 
  ils doivent être les seuls à pour y accéder.


- Lors de la création de ton instance RDS, à partir du service RDS,
  tu dois définir un type de stockage et l'espace disque pour tes données.
  Et devine le service qui se cache derrière la gestion de l'espace disque ?  
  Eh bah ! C'est le service EBS, bien sûr 😄.

  Par contre, tu ne peux pas accéder à disque directement depuis le service EBS. 
  Car encore une fois, pour qu'AWS garantisse toutes les notions citées ci-dessus,
  ils ne peuvent pas laisser les utilisateurs accéder aux disques directement.
---
...
- des sauvegardes complètes sont faites par RDS tous les jours au moment du créneau de maintenance (que tu définis à la création).
- des sauvegardes des `logs de transactions` sont faites tous les 5 minutes.
...
les sauvegardes qui ont plus de 7 jours sont supprimés automatiquement par RDS.

Le dernier type de sauvegarde, sont les sauvegardes manuelles aussi appelées `DB Snapshots`, 
...
Car tiens-toi bien, le service RDS prend aussi en charge le monitoring, 
et devine avec quel service il fait ça ?  
Oui, c'est bien ça, RDS utile AWS CloudWatch pour le monitoring.

Donc pour que ce service sache à quel moment augmenter la taille de stockage, 
...
Et on sait de manière générale que ces sont les opérations de lecture qui impacteront 
...
Ensuite une réplication asynchrone entre l'instance RDS master 
et la/les instance(s) `Read Replicas` se déclenche. 
La réplication prend quelques millièmes de seconde.
...
- dispatcher dans toutes les zones d'une région, dans de cas `Cross AZ Read Replicas`,
- ou encore dispatcher sur plusieurs régions.

---
#### ⚠ À ne pas oublier.
Comme l'opération asynchrone n'est pas instantané. AWS mets en garde les utilisateurs
...
si à chaque fois, tu es obligé définir le type de connection à utiliser pour chaque opération.

C'est pour cette raison que les `Read Replicas` sont principalement utilisés que 
...
**Le but étant d'utiliser le `Read Replicas` pour ne pas impacter ton instance RDS master qui sert pour l'écriture des opérations.**
...
une instance RDS en "attente" où chaque opération d'écriture sur l'instance RDS master est répliquée de manière **synchrone**.
Comme ça en cas chute de l'instance master, l'instance en "attente" 
prendra le relais et deviendra l'instance master, ce processus est appelé `automatic failover`.
...
je t'invite à utiliser AWS calculator pour un prix plutôt exact selon ton besoin.
...
Étant donné que c'est un service couteux, tu n'es pas obligé de l'utiliser dans le projet du jour.
Cependant, il est important que tu puisses conseiller tes futurs clients/ta team 
sur ce service au cas où, vous serez amener à choisir ou non d'utiliser une DB managé 😉.  
De ce fait, je tiens quand même à ce que tu voies de tes propres yeux 
à quoi ressemble ce service avec ce [tutorial](TODO : faire une video).


## 3. Points importants à retenir
Voici les éléments à garder en tête :
- Amazon RDS, est un service qui offre des DB managées et parce qu'ils sont managés, 
  la plupart la quasi-totalité des opérations
  de maintenance, disponibilités et sauvegardes sont gérées par AWS à ta place.
- ce service permet l'`automatic failover`
- avec ce service des sauvegardes automatiques et manuelles sont possibles 
